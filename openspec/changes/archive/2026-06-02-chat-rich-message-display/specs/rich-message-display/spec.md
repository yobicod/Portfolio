...
The system SHALL support a typed message content model (`RichMessage`) that extends plain-text messages with structured variants: `timeline`, `list`, and `contact`. Each variant carries typed data instead of a raw string.
...
- **WHEN** the bot responds to About me, Project, Tech stack, or unknown queries
